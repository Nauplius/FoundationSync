        ...
        internal static void BuildReport(string logMessage, LoggingExType logType)
        {
            ...
            switch (logType)
            {
                case LoggingExType.UsersFoundCount:
                    _usersFoundMemoryStream.Write(bytes, 0, bytes.Length);
                    _usersFoundMemoryStream.Flush();
                    break;
                case LoggingExType.UsersUpdatedCount:
                    _usersUpdatedMemoryStream.Write(bytes, 0, bytes.Length);
                    _usersUpdatedMemoryStream.Flush();
                    break;
                case LoggingExType.UsersDeletedCount:
                    _usersDeletedMemoryStream.Write(bytes, 0, bytes.Length);
                    _usersDeletedMemoryStream.Flush();
                    break;
                case LoggingExType.UserProperties:
                    _userPropertiesMemoryStream.Write(bytes, 0, bytes.Length);
                    _userPropertiesMemoryStream.Flush();
                    break;
            }
